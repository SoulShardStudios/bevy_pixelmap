...
fn main(@builtin(global_invocation_id) invocation_id: vec3<u32>) {
    ...
    for (var i: u32 = 0; i < 8; i = i + 1) {
        ...
        var possey = input_texture_pos - source_pos + vec2<i32>(coords.x, coords.y * -1 + i32(input_texture_size.y));
        var possey2 = vec2<i32>(possey.x, possey.y);

        var source_pixel: vec4<f32> = vec4<f32>(0.0);

        if (possey2.x >= 0 && possey2.x < i32(source_size.x) &&
            possey2.y >= 0 && possey2.y < i32(source_size.y)) {
